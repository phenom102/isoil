                        ...
                        while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                            ...
                            if(isset($coordinate[$tag])) {
                                ...
                                $punti_mappa[] = [
                                        ...
                                        "lat" => $coordinate[$tag]["x"],
                                        "lng" => $coordinate[$tag]["y"]
                                ];
                            }
                        }
...
<script>
    ...
    // Inizializza mappa centrata (modifica con la tua zona)
    const map = L.map('map').setView([45.0000, 9.0000], 13);
    ...
    // Array per adattare zoom automatico
    let bounds = [];
    ...
    puntiMappa.forEach(punto => {
        ...
        if (punto.lat && punto.lng) {
            ...
            marker.bindPopup(`
            <strong>${punto.tag}</strong><br>
            ${punto.descrizione}<br>
            <b>${punto.valore} l/s</b>
        `);
    ...
    // Zoom automatico per mostrare tutti i punti
    if (bounds.length > 0) {
