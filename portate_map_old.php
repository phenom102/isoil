...
$coordinate = [];
$righe = file(__DIR__ . "/coordinate.txt", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
foreach ($righe as $riga) {
    ...
    if(count($parti) >= 3){
        ...
        $tag = strtoupper(trim($parti[0]));
        $x = floatval(trim($parti[1]));
        $y = floatval(trim($parti[2]));
...
<nav class="px-nav px-nav-left">
    ...
    <ul class="px-nav-content">
        ...
        <li class="px-nav-box b-t-1 p-a-2">
            <a href="portate.php" class="btn btn-primary btn-block btn-outline"><i class="px-nav-icon fa fa-tint"></i>Portate ingr. serb.</a>
        </li>
        <li class="px-nav-box b-t-1 p-a-2">
            <a href="portate_map.php" class="btn btn-primary btn-block btn-outline"><i class="px-nav-icon fa fa-map"></i>Portate ingr. serb. Mappa</a>
        </li>
    </ul>
</nav>
    ...
    <div class="page-header">
        <div class="row">
            <div class="col-md-4 text-xs-center text-md-left text-nowrap">
                <h1><i class="page-header-icon ion-ios-pulse-strong"></i>Dashboard Portate Ingressi Serbatoi Mappa</h1>
            </div>
                ...
                <table class="table table-striped table-bordered" id="datatables">
                    ...
                    if ($conn){
                        ...
                        while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                            ...
                            if(isset($coordinate[$tag])) {

                                $punti_mappa[] = [
                                        "tag" => $tag,
                                        "descrizione" => $row['Descrizione'],
                                        "valore" => floatval($row['Valore']),
                                        "x" => $coordinate[$tag]["x"],
                                        "y" => $coordinate[$tag]["y"]
                                ];
                            }
...
print_r($coordinate);
    ...
    <div id="mappa-wrapper">
        <div id="mappa-container">
            <img src="mappa_area.jpg" id="mappa-img">
        </div>
    </div>
</div>
...
<script>
    ...
    const puntiMappa = <?php echo json_encode($punti_mappa); ?>;

    puntiMappa.forEach(punto => {

        const marker = document.createElement("div");
        marker.className = "marker";
        marker.style.left = punto.x + "%";
        marker.style.top = punto.y + "%";

        const tooltip = document.createElement("div");
        tooltip.className = "tooltip-custom";
        tooltip.innerHTML = `
        <strong>${punto.descrizione}</strong><br>
        <strong>${punto.tag}</strong>
        Portata: ${punto.valore} l/s
    `;
